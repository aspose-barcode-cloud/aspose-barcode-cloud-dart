...
var api_instance = new BarcodeApi();
var type = type_example; // String | Type of barcode to generate.
var text = text_example; // String | Text to encode.
var twoDDisplayText = twoDDisplayText_example; // String | Text that will be displayed instead of codetext in 2D barcodes. Used for: Aztec, Pdf417, DataMatrix, QR, MaxiCode, DotCode
var textLocation = textLocation_example; // String | Specify the displaying Text Location, set to CodeLocation.None to hide CodeText. Default value: CodeLocation.Below.
var textAlignment = textAlignment_example; // String | Text alignment.
var textColor = textColor_example; // String | Specify the displaying CodeText's Color. Default value: Color.Black.
...
var noWrap = true; // bool | Specify word wraps (line breaks) within text. Default value: false.
var resolution = 1.2; // double | Resolution of the BarCode image. One value for both dimensions. Default value: 96 dpi.
var resolutionX = 1.2; // double | DEPRECATED: Use 'Resolution' instead.
var resolutionY = 1.2; // double | DEPRECATED: Use 'Resolution' instead.
var dimensionX = 1.2; // double | The smallest width of the unit of BarCode bars or spaces. Increase this will increase the whole barcode image width. Ignored if AutoSizeMode property is set to AutoSizeMode.Nearest or AutoSizeMode.Interpolation.
var textSpace = 1.2; // double | Space between the CodeText and the BarCode in Unit value. Default value: 2pt. Ignored for EAN8, EAN13, UPCE, UPCA, ISBN, ISMN, ISSN, UpcaGs1DatabarCoupon.
var units = units_example; // String | Common Units for all measuring in query. Default units: pixel.
var sizeMode = sizeMode_example; // String | Specifies the different types of automatic sizing modes. Default value: AutoSizeMode.None.
var barHeight = 1.2; // double | Height of the barcode in given units. Default units: pixel.
var imageHeight = 1.2; // double | Height of the barcode image in given units. Default units: pixel.
var imageWidth = 1.2; // double | Width of the barcode image in given units. Default units: pixel.
var rotationAngle = 1.2; // double | BarCode image rotation angle, measured in degree, e.g. RotationAngle = 0 or RotationAngle = 360 means no rotation. If RotationAngle NOT equal to 90, 180, 270 or 0, it may increase the difficulty for the scanner to read the image. Default value: 0.
var backColor = backColor_example; // String | Background color of the barcode image. Default value: Color.White.
var barColor = barColor_example; // String | Bars color. Default value: Color.Black.
var borderColor = borderColor_example; // String | Border color. Default value: Color.Black.
var borderWidth = 1.2; // double | Border width. Default value: 0. Ignored if Visible is set to false.
var borderDashStyle = borderDashStyle_example; // String | Border dash style. Default value: BorderDashStyle.Solid.
var borderVisible = true; // bool | Border visibility. If false than parameter Width is always ignored (0). Default value: false.
...
var filledBars = true; // bool | Value indicating whether bars are filled. Only for 1D barcodes. Default value: true.
var alwaysShowChecksum = true; // bool | Always display checksum digit in the human readable text for Code128 and GS1Code128 barcodes.
...
var supplementData = supplementData_example; // String | Supplement parameters. Used for Interleaved2of5, Standard2of5, EAN13, EAN8, UPCA, UPCE, ISBN, ISSN, ISMN.
var supplementSpace = 1.2; // double | Space between main the BarCode and supplement BarCode.
...
var type = type_example; // String | The type of barcode to read.
...
var detectEncoding = true; // bool | A flag which force engine to detect codetext encoding for Unicode.
var preset = preset_example; // String | Preset allows to configure recognition quality and speed manually. You can quickly set up Preset by embedded presets: HighPerformance, NormalQuality, HighQuality, MaxBarCodes or you can manually configure separate options. Default value of Preset is NormalQuality.
var rectX = 56; // int | Set X for area for recognition.
var rectY = 56; // int | Set Y for area for recognition.
var rectWidth = 56; // int | Set Width of area for recognition.
var rectHeight = 56; // int | Set Height of area for recognition.
var stripFNC = true; // bool | Value indicating whether FNC symbol strip must be done.
var timeout = 56; // int | Timeout of recognition process.
var medianSmoothingWindowSize = 56; // int | Window size for median smoothing. Typical values are 3 or 4. Default value is 3. AllowMedianSmoothing must be set.
var allowMedianSmoothing = true; // bool | Allows engine to enable median smoothing as additional scan. Mode helps to recognize noised barcodes.
var allowComplexBackground = true; // bool | Allows engine to recognize color barcodes on color background as additional scan. Extremely slow mode.
var allowDatamatrixIndustrialBarcodes = true; // bool | Allows engine for Datamatrix to recognize dashed industrial Datamatrix barcodes. Slow mode which helps only for dashed barcodes which consist from spots.
var allowDecreasedImage = true; // bool | Allows engine to recognize decreased image as additional scan. Size for decreasing is selected by internal engine algorithms. Mode helps to recognize barcodes which are noised and blurred but captured with high resolution.
var allowDetectScanGap = true; // bool | Allows engine to use gap between scans to increase recognition speed. Mode can make recognition problems with low height barcodes.
var allowIncorrectBarcodes = true; // bool | Allows engine to recognize barcodes which has incorrect checksum or incorrect values. Mode can be used to recognize damaged barcodes with incorrect text.
var allowInvertImage = true; // bool | Allows engine to recognize inverse color image as additional scan. Mode can be used when barcode is white on black background.
var allowMicroWhiteSpotsRemoving = true; // bool | Allows engine for Postal barcodes to recognize slightly noised images. Mode helps to recognize slightly damaged Postal barcodes.
var allowOneDFastBarcodesDetector = true; // bool | Allows engine for 1D barcodes to quickly recognize high quality barcodes which fill almost whole image. Mode helps to quickly recognize generated barcodes from Internet.
var allowOneDWipedBarsRestoration = true; // bool | Allows engine for 1D barcodes to recognize barcodes with single wiped/glued bars in pattern.
var allowQRMicroQrRestoration = true; // bool | Allows engine for QR/MicroQR to recognize damaged MicroQR barcodes.
var allowRegularImage = true; // bool | Allows engine to recognize regular image without any restorations as main scan. Mode to recognize image as is.
var allowSaltAndPepperFiltering = true; // bool | Allows engine to recognize barcodes with salt and pepper noise type. Mode can remove small noise with white and black dots.
var allowWhiteSpotsRemoving = true; // bool | Allows engine to recognize image without small white spots as additional scan. Mode helps to recognize noised image as well as median smoothing filtering.
var checkMore1DVariants = true; // bool | Allows engine to recognize 1D barcodes with checksum by checking more recognition variants. Default value: False.
var fastScanOnly = true; // bool | Allows engine for 1D barcodes to quickly recognize middle slice of an image and return result without using any time-consuming algorithms. Default value: False.
...
var scanWindowSizes = []; // List<int> | Scan window sizes in pixels. Allowed sizes are 10, 15, 20, 25, 30. Scanning with small window size takes more time and provides more accuracy but may fail in detecting very big barcodes. Combining of several window sizes can improve detection quality.
var similarity = 1.2; // double | Similarity coefficient depends on how homogeneous barcodes are. Use high value for for clear barcodes. Use low values to detect barcodes that ara partly damaged or not lighten evenly. Similarity coefficient must be between [0.5, 0.9]
...
var supplementSpace = 1.2; // double | Space between main the BarCode and supplement BarCode.
var barWidthReduction = 1.2; // double | Bars reduction value that is used to compensate ink spread while printing.
var storage = storage_example; // String | Image's storage.
...
var generatorParamsList = new GeneratorParamsList(); // GeneratorParamsList | List of barcodes
var format = format_example; // String | Format of file
