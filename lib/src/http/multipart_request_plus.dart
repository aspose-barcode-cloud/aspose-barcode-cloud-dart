...
import 'package:http/http.dart' show BaseRequest, ByteStream, MultipartFile;
...
/// Copy of multipart_request.dart can handle multiple form values for one key
/// Supports form with multi choice fields
///
...
class MultipartRequestPlus extends BaseRequest {
  ...
  /// Creates a new [MultipartRequestPlus].
  MultipartRequestPlus(super.method, super.url);
  ...
  Stream<List<int>> _finalize(String boundary) async* {
    ...
    for (var field in fields.entries) {
      for (final oneValue in field.value) {
        yield separator;
        yield utf8.encode(_headerForField(field.key, oneValue));
        yield utf8.encode(oneValue);
        yield line;
      }
    }
